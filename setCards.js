// Set card SVG definitions - injects into DOM when loaded
function injectSetCardSVG() {
    // Check if SVG definitions already exist
    if (document.getElementById('set-card-definitions')) {
        return;
    }

    const svgDefinitions = `
        <svg id="set-card-definitions" style="position: fixed; z-index: -1; width: 0; height: 0;">
            <defs>
                <path
                    id="squiggle"
    ...
    document.body.insertAdjacentHTML('beforeend', svgDefinitions);
}

// Auto-inject when DOM is ready
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', injectSetCardSVG);
} else {
    injectSetCardSVG();
}
